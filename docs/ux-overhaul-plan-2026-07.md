...
4. [x] **Location dual-model collapsed — code-complete on dev 2026-07-04, staged S0–S4.** The equipment collection is now the single source of truth for equipment↔gym; the second representation (`location.equipment[]` catalog refs) and its render-time healing are gone.
   - **S0** (bf0434a) `location-id-resolver` + `planLocationIdBackfill`/`planOrphanGymDocs` planners + 11 tests. Distinguishes AMBIGUOUS (2+ docs share a name) from orphan (name-only gym, no doc).
   - **S1** (c1e1f5a) additive dual-write `equipment.locationIds[]` at every A-writer (`_deriveLocationIds` + a warm `_cachedLocations`; active-workout reads the session cache — no hot-path fetch; guarded so a cold cache never clobbers ids).
   - **S2** (1483b1b runner) backfill **APPLIED + verified** via `runLocationIdBackfill({apply:true})`: 92/92 equipment docs stamped, all exact matches, names untouched, 2 orphans (Bellagio/Work) correctly left name-only.
   - **S3** (46b8556) `getEquipmentAtLocation` matches id-first with a name-fallback union.
   - **S4** (04116d3, 6509fa5) gym-detail list + gym-card counts read the equipment collection alone (fixes the **double-count**); catalog-detail/toggle/quick-add readers derive owned-catalog-refs from equipment; all B writers removed (kept the A-side promote/untag); deleted `healDuplicateLocationEquipment` / `migrateLocationCatalogRefs` / `syncCatalogRefOnLocation` + the manager's 4 `*LocationEquipment*` methods + `saveLocation`'s `equipment[]` passthrough; **`deleteLocation` now strips the gym from all equipment** (fixes the deleted-gym-haunt). `promoteCatalogToEquipment` + `untagGymFromPromotedDocs` kept (they're the A-side add/remove). *Kept: `renameLocationOnEquipment` (name cascade — ids are stable, but names stay fresh for display).* **Open: dev smoke-test of catalog/gym flows → prod; inert `location.equipment[]` data on 2 docs can be stripped (optional).**
