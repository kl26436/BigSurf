...
2. **Give catalog adds an exercise mapping.** Best: add `exercises: []` to catalog entries over time. Pragmatic first pass: on add, fuzzy-match the machine name against the exercise library ("Iso-Lateral Bench Press" → "Bench Press") and offer body-part-based suggestions for the rest ("What do you use this for?" — one tap from a short list). Without this, pickers and planner stay half-blind.
3. **One shared picker sheet** (per the CLAUDE.md `openShared*` pattern) with a search field, used by both the workout and template flows, with "Add from catalog" opening the same quick-add sheet parameterized with the session gym. That makes the mid-workout flow: tap Change → search catalog → tap machine → it's created, gym-tagged, exercise-linked, and selected. That's the "super intuitive" travel flow.
