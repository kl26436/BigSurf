...
- [x] **§2 Body Measurements entry rewrite** (M) — minimum-viable delta applied (full-page structural rewrite skipped to avoid breaking the shipped modal): `bm-entry-body` + `bm-unit-chips` classes replace inline styles; muscle-mass row added to the composition section; `muscleMass` value plumbed through `saveBodyWeightEntry()`.
- [x] **§3 Manual Workout restyle** (M) — 15+ inline styles extracted into a new [pages/manual-workout.css](styles/pages/manual-workout.css): `.manual-exercise-card`, `.manual-ex-head`, `.manual-ex-drag`, `.manual-ex-name-col`, `.manual-ex-name`, `.manual-ex-equip`, `.manual-ex-overflow`, `.manual-sets-table__*` (col-num/weight/reps/check, num, input, check), `.manual-add-set-btn`. Registered in index.css. Persistence (`isManual: true`) untouched.
- [x] **§9b Location Edit build** (M) — shipped as part of Phase D deferred work. Location detail page uses `.page-header` (section-header-row), `.loc-map-card__*`, `.loc-radius-chips` for the 100m/500m/1km match-radius chips, `.link-row` (.srow-equivalent) for equipment list, `.danger-action-row` + `.danger-action-btn` for delete. Window-bindings already in place (`updateLocationRadius`, `detectAndAddLocation`, `deleteLocation`, `openLocationDetail`).
- [~] **§7 DEXA upload + detail** (L) — inline styles audited: only 2 remain, both are truly dynamic (width %, color per data value) and valid per CLAUDE.md Rule 8. `.dexa-insight-card` exists in dexa.css. Full structural rewrite (upload modal → full-page `.dexa-drop` card + 2×2 `.dexa-stat-grid`) **deferred** — requires UX validation on the upload flow before ripping out the shipped modal. Add `.dexa-drop` / `.dexa-stat-grid` families to dexa.css when doing the rewrite.
- [x] **§8 AI Coach chat polish** (L, inline-style pass only) — 7 inline styles in [ai-coach-ui.js](js/core/features/ai-coach-ui.js) extracted: `.prompt-icon--warning` / `.prompt-icon--warm` / `.prompt-icon--core` for the 3 tinted prompt cards; `.coach-msg-icon` + existing `.text-warning` / `.text-muted` utilities for error-message icons. CSS added to ai-coach.css. Full structural rewrite (`.coach-hero`, `.coach-msg--user/--bot`, `.coach-input-bar`, `.coach-thinking`) **deferred** — 627 LOC CSS rewrite is risky without user validation.
