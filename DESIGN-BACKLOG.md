...
- [x] **Header-meta elapsed-time promotion** — header title restructured into 3 lines: workout name (muted `--font-xs`), **elapsed (`--font-md` weight 800, hero)**, Exercise N/M (`--font-2xs` muted below). `min-height: 48px → 56px` to accommodate. Timer tick updates just the elapsed textContent (no meta re-render). Renders: [active-workout-ui.js:138-142](js/core/workout/active-workout-ui.js#L138), [active-workout-v2.css:38-61](styles/pages/active-workout-v2.css#L38).
...
- [x] **New-equipment chip defaults from exercise category** — duplicate of the Phase B item. Shipped via `guessEquipmentType()` at [active-workout-ui.js:1319](js/core/workout/active-workout-ui.js#L1319); wired into `awQuickAddEquipment()` at [:1280](js/core/workout/active-workout-ui.js#L1280). Pull-up/push-up/dip/plank → Bodyweight; cable/pulldown → Cable; dumbbell/curl/fly → Dumbbell; barbell/squat/deadlift/bench → Barbell; cardio → Machine; core → Bodyweight; otherwise Machine.
