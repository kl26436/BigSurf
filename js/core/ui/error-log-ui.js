...
/**
 * Export all errors (session + persisted) as a JSON file download.
 */
export async function exportErrorLog() {
    // Load persisted errors if not already loaded
    let saved = persistedErrors;
    if (saved.length === 0) {
        saved = await loadPersistedErrors();
    }
    const session = getErrorLog();

    const exportData = {
        exportedAt: new Date().toISOString(),
        sessionErrors: session,
        persistedErrors: saved,
        totalCount: session.length + saved.length,
    };
    ...
    a.href = url;
    a.download = `bigsurf-errors-${new Date().toISOString().slice(0, 10)}.json`;
